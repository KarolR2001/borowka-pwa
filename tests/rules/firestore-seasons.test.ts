...
describe("Firestore season rules", () => {
  it("rejects season reads for anonymous and blocked users", async () => {
    await seedProfiles(
      profile({
        uid: "blocked-1",
        active: false,
  ...
  it("allows approved active profiles to read seasons", async () => {
    await seedProfiles(
      profile({
        uid: "admin-1",
        role: "ADMIN"
    ...
    const snapshot = await assertSucceeds(getDocs(collection(operatorDb, "seasons")));
